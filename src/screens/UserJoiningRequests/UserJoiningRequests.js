...
export default class UserNotification extends React.Component {
  ...
  acceptRequest(Number, name, data, key) {
    ...
    firebase
      .database()
      .ref("app/"+screenProps.user.CommunityName+"/User/" + data)
      .set({
        Name: name,
        Email: "",
        Profile_photo:
    ...
    firebase
      .database()
      .ref("app/"+screenProps.user.CommunityName+"/User/" + data + "/Business_details")
      .update({
        Name: "",
        Contact_Number: "",
        Type: "",
    ...
    firebase
      .database()
      .ref("app/"+screenProps.user.CommunityName+"/Joining_Requests/" + data)
      .remove();
  ...
  getDataFromFirebase() {
    ...
    firebase
      .database()
      .ref("app/"+screenProps.user.CommunityName+"/Joining_Requests")
      .on("child_added", data => {
        var result = [];
        var key1 = [];
        key1.push(data.key);
