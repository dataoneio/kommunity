...
export default class BroadcastedPost extends React.Component {
  ...
  getDataForBroadcast(eventid, arr1) {
    ...
    firebase
      .database()
      .ref(
        "app/" + screenProps.user.CommunityName + "/Event details/" + eventid
      )
      .on("value", data => {
  ...
  componentDidMount() {
    ...
    firebase
      .database()
      .ref("app/" + screenProps.user.CommunityName + "/BroadcastPost")
      .on(
        "child_added",
        data => {
    ...
    if(this.state.results.length == 0)
    {
      return(
        <View>
          ...
          <Text style={{padding:10,fontSize:16,fontWeight:"bold",textAlign:"center"}}>No special posts for you...!!!</Text>
